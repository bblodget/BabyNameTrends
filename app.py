...
def load_data(directory, name, year, comparison):
    ...
    for filename in os.listdir(directory):
        if filename.startswith('yob') and filename.endswith('.txt'):
            ...
            if (comparison == 'before' and file_year < year) or \
               (comparison == 'equal' and file_year == year) or \
               (comparison == 'after' and file_year > year):
                # Read the data from the file
                yearly_data = pd.read_csv(os.path.join(directory, filename), names=["Name", "Gender", "Count"])
...
def main():
    ...
    year = st.number_input("Enter the year:", min_value=1880, max_value=2023, value=1970)
    ...
    if os.path.exists(directory):
        ...
        st.write(f'Total number of babies named {name} {comparison} {year}: {total_babies}')
